...
def _doScan():
        ...
        if(cert):
            ...
            for line in oldSpec.splitlines():
                if "ssltrust0".encode('utf-8') in line:
                    ...
                    break
            o = hashlib.sha1()
            o.update(base64.decodebytes(oldcert.encode('utf-8')))
            n = hashlib.sha1()
            n.update(base64.decodebytes(newcert_parsed.encode('utf-8')))
    ...
    if len(mismatchedIDs) !=0:
        logger.warning("***WARNING*** %d Mismatched ID(s) found", len(mismatchedIDs))
        ...
        logger.warning("Please DOUBLE CHECK the detection before running 'fix'")
        logger.warning("NOTE: Partial upgrade state of 5.5 to 6.x is unsupported for this tool- 5.5 web client registration might change")
        logger.info("")
    else:
